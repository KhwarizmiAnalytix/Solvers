...
(SOLVERS_ENABLE_*/SOLVERS_* in CMakeLists.txt) — there is no multi-module
Library/* tree or --project.* scoping here, just one flat set of flags plus
Solvers's own SOLVERS_BACKEND selector (NATIVE, LOGURU, GLOG, SPDLOG).
...
class ErrorLogger:
    """Centralized error solvers system for comprehensive error tracking."""

    def __init__(self, log_dir: str = "logs"):
        self.log_dir = Path(log_dir)
        self.log_dir.mkdir(exist_ok=True)
        self.log_file = (
            self.log_dir / f"solvers_build_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
        )
        self.errors = []
...
class SolversFlags:
    """Maps setup.py dotted tokens to Solvers's SOLVERS_* CMake cache variables.

    Scoped 1:1 to the options CMakeLists.txt actually defines — unlike XSigma's
    XSigmaFlags there is no multi-module fan-out (Solvers is a single flat
    CMake target), no GPU/MKL/vectorization backends, and no --project.*
    scoping. Solvers does own a real backend selector (SOLVERS_BACKEND: NATIVE,
    ...
    def __initialize_flags(self):
        ...
        self.__description = [
            ...
            "build the Solvers test suite (SOLVERS_ENABLE_TESTING; default ON)",
            "build Solvers example programs",
            "enable GoogleTest for Solvers (default ON; token disables it)",
            "enable Google Benchmark for Solvers (default ON; token disables it)",
            ...
            "solvers backend: native, loguru (default), glog, or spdlog",
    ...
    def __build_cmake_flag(self):
        ...
        self.__name = {
            ...
            "examples": "SOLVERS_ENABLE_EXAMPLES",
            "gtest": "SOLVERS_ENABLE_GTEST",
            "benchmark": "SOLVERS_ENABLE_BENCHMARK",
            "magicenum": "SOLVERS_ENABLE_MAGICENUM",
            "cxademangle": "SOLVERS_ENABLE_CXA_DEMANGLE",
            "portablefloat": "SOLVERS_ENABLE_PORTABLE_FLOAT_FORMAT",
            "stdformat": "SOLVERS_FORMAT_USE_STD",
            "exceptionmode": "SOLVERS_DEFAULT_EXCEPTION_MODE",
            "backend": "SOLVERS_BACKEND",
            "icecc": "SOLVERS_ENABLE_ICECC",
            "cache": "SOLVERS_ENABLE_CACHE",
            "cache_type": "SOLVERS_CACHE_BACKEND",
            "clangtidy": "SOLVERS_ENABLE_CLANGTIDY",
            "fix": "SOLVERS_ENABLE_FIX",
            "iwyu": "SOLVERS_ENABLE_IWYU",
            "sanitizer": "SOLVERS_ENABLE_SANITIZER",
            "sanitizer_enum": "SOLVERS_SANITIZER_TYPE",
            "spell": "SOLVERS_ENABLE_SPELL",
            "valgrind": "SOLVERS_ENABLE_VALGRIND",
            "linker": "SOLVERS_LINKER_CHOICE",
        ...
        self.__value.update(
            {
                ...
                "test": self.ON,  # SOLVERS_ENABLE_TESTING default ON
                "gtest": self.ON,  # SOLVERS_ENABLE_GTEST default ON
                "benchmark": self.ON,  # SOLVERS_ENABLE_BENCHMARK default ON
                "magicenum": self.ON,  # SOLVERS_ENABLE_MAGICENUM default ON
                "cache": self.ON,  # SOLVERS_ENABLE_CACHE default ON
    ...
    def __process_arg_list(self, arg_list):
        ...
        for arg in arg_list:
            if arg == "static":
                ...
                self.builder_suffix += f"_{arg}"
                print_status(f"Selecting solvers backend: {arg.upper()}", "INFO")
            elif arg == "throw":
    ...
    def __validate_flags(self):
        ...
        if backend and backend not in ["NATIVE", "LOGURU", "GLOG", "SPDLOG"]:
            print_status(f"Invalid solvers backend '{backend}'. Valid options: native, loguru, glog, spdlog", "ERROR")
            sys.exit(1)
        ...
        if self.__value.get("stdformat") == self.ON:
            ...
            if int(cxxstd) < 20:
                print_status(
                    f"stdformat requires C++20 or newer (got cxx{cxxstd}); SOLVERS_FORMAT_USE_STD will fail to configure.",
                    "ERROR",
                )
                sys.exit(1)
    ...
    for arg in args:
        if arg.startswith("--sanitizer."):
            ...
            if backend_value in valid_backends:
                ...
                print_status(f"Invalid solvers backend: {backend_value}. Valid options: {', '.join(valid_backends)}", "ERROR")
                sys.exit(1)
        elif arg.startswith("--linker."):
...
def main():
    if len(sys.argv) == 2 and sys.argv[1] == "--help":
        ...
        print("  Backend:      Loguru (SOLVERS_BACKEND default)")
        print("=" * 80)
        print("\nUsage examples:")
        ...
        print("  5. Build against a specific solvers backend:")
