...
#include "optimization_algorithm/solver_enum.h"
 ...
 * auto objective_function = [&](const vector<double>& x, vector<double>& residuals) {
 *     // Your objective function implementation
 * };
 * 
 * auto objective_function_aad = [&](const vector<double>& x, matrix<double>& jacobian) {
...
public:
    ...
    using objective_function_type     = std::function<void(const vector<double>&, vector<double>&)>;
    using objective_function_aad_type = std::function<void(const vector<double>&, matrix<double>&)>;
