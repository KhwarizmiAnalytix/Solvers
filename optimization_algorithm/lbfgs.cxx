...
class line_search<lbfgs_line_search_type::NOCEDAL_WRIGHT>
{
    ...
    using vector_type   = vector<scalar_type>;
...
public:
        ...
        const solver_options_bfgs& param)
    {
        ...
        const auto dg_init   = -quarisma::accumulate(grad * direction);
        QUARISMA_CHECK(dg_init <= 0, "the moving direction increases the objective function value");
        ...
        for (; iter < param.max_iteration_linesearch(); ++iter)
        {
            ...
            const scalar_type dg = -quarisma::accumulate(grad * direction);
...
public:
        ...
        const solver_options_bfgs& param)
    {
        ...
        QUARISMA_CHECK(step > scalar_type(0), "'step' must be positive");

        const scalar_type fx_init = fx;
        const scalar_type dg_init = -quarisma::accumulate(grad * direction);
...
public:
        ...
        const solver_options_bfgs& param)
    {
        ...
        for (size_type iter = 0; iter < param.max_iteration_linesearch(); ++iter)
        {
            ...
            QUARISMA_CHECK(step_lo < step_hi, "step min is bigger than step max");
...
optimization_algorithm_output LBFGS::solve(
    vector<double>& parameters, const solver_options_bfgs& options) const
{
    ...
    for (; iter < options.max_num_iterations(); ++iter)
    {
        ...
        v[iter_tau] = p_new - parameters;
        r[iter_tau] = grad - grad_old;

        q.deepcopy(grad);

        for (size_type j = 0; j <= iter_tau; ++j)
        {
            alpha[j] = accumulate(v[j] * q) / accumulate(v[j] * r[j]);
            q        = q - alpha[j] * r[j];
        }
        const auto v_tau = v[iter_tau];
        const auto r_tau = r[iter_tau];

        q = q * accumulate(v_tau * r_tau) / accumulate(r_tau * r_tau);
        ...
        for (size_type j = 0; j <= iter_tau; ++j)
        {
            ...
            q = q +
                (alpha[offset] - accumulate(r[offset] * q) / accumulate(v[offset] * r[offset])) *
                    v[offset];
        }

        scalar_type sign = 1;
        sign             = std::copysign(sign, accumulate(q * grad));
