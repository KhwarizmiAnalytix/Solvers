      ...
      upper_bounds_(std::move(ub))
{
    if (objfun_aad_ == nullptr)
    {
        ...
        objfun_aad_ = [this, bump](vector<double> const& x, matrix<double>& dy_dx)
        {
...
void nlopt_solver::solve(
    ...
    QUARISMA_UNUSED const solver_options_nlopt& options)
{
#if QUARISMA_MODULE_ENABLE_QUARISMA_nlopt
...
double nlopt_solver::OBJFUN(const std::vector<double>& x, std::vector<double>& grad, void* data)
{
    ...
    vector<double> f_tmp(self->num_residuals_);

    self->objfun_(x_tmp, f_tmp);

    double result = accumulate(sqr(f_tmp));

    if (!grad.empty())
    {
        vector<double> grad_tmp(grad);

        matrix<double> f_aad_tmp(self->num_residuals_, self->num_parameters_);
        self->objfun_aad_(x_tmp, f_aad_tmp);
        grad_tmp = 2. * (f_tmp * f_aad_tmp);
    }
...
double nlopt_solver::CONFUN(
    QUARISMA_UNUSED const std::vector<double>& x,
    QUARISMA_UNUSED std::vector<double>& grad,
    QUARISMA_UNUSED void*                data)
{
    //auto*          self = static_cast<nlopt_solver*>(data);
    //vector<double> x_tmp(x);
    //vector<double> grad_tmp(grad);
