...
#if QUARISMA_MODULE_ENABLE_QUARISMA_ceres
...
public:
    LambdaCostFunctor(
        ceres_solver_algorithms::CostFunctionLambda     cost_function,
        ceres_solver_algorithms::CostFunctionLambda_aad cost_function_aad,
        int                                             num_parameters,
        int                                             num_residuals)
          ...
          num_residuals_(num_residuals)
    {
        ...
        mutable_parameter_block_sizes()->push_back(num_parameters_);
    }

    bool Evaluate(
        double const* const* parameters, double* residuals, double** jacobians) const override
    {
        quarisma::vector<double> params_double(parameters[0], num_parameters_);

        quarisma::vector<double> residual_values(residuals, num_residuals_);
        cost_function_(params_double, residual_values);

        if (jacobians != nullptr && jacobians[0] != nullptr)
        {
            matrix<double> grad(jacobians[0], num_residuals_, num_parameters_);
...
private:
    ceres_solver_algorithms::CostFunctionLambda     cost_function_;
    ceres_solver_algorithms::CostFunctionLambda_aad cost_function_aad_;
    int                                             num_residuals_;
    int                                             num_parameters_;
};
...
bool ceres_solver_algorithms::is_supported()
{
#if QUARISMA_MODULE_ENABLE_QUARISMA_ceres
    return true;
#else
...
bool ceres_solver_algorithms::solve(
    std::vector<double>& parameters, const solver_options_ceres& options)
{
    QUARISMA_CHECK(parameters.size() == num_parameters_);
    ...
    if (cost_function_aad_ == nullptr)
    {
        ...
        cost_function_aad_ = [this, bump](vector<double> const& x, matrix<double>& dy_dx)
        {
            ...
            vector<double> y_plus(number_of_targets);
            vector<double> y_minus(number_of_targets);

            vector<double> x_tmp(number_of_parameters);
            x_tmp.deepcopy(x);
            ...
            for (size_t i = 0; i < number_of_parameters; ++i)
            {
                ...
                {
                    dy_dx[j][i] = (y_plus[j] - y_minus[j]) * inv_dx;
                }
    ...
    if (options.debug())
    {
        QUARISMA_LOG_INFO("ceres solver summary: " << summary.BriefReport());
    }

    return summary.IsSolutionUsable();
#else
    QUARISMA_NOT_IMPLEMENTED("ceres solver not supported");
#endif
}
}  // namespace quarisma
