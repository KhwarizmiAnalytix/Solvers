...
class interpolator_geometric_average : public interpolator_linear<Container, T, S>
{
...
public:
     ...
     */
    interpolator_geometric_average(std::vector<T> x_points, Container y_points)
        : base(std::move(x_points), transform_y_values(std::move(y_points), x_points))
    {
        x_ref_ = this->x_[0];
    }
    ...
    S interpolate(const T& t) const override
    {
        ...
        return (t - x_ref_) * base::interpolate(t);
    }
...
private:
    ...
    static Container transform_y_values(Container y_points, const std::vector<T>& x_points)
    {
        if constexpr (std::is_arithmetic<S>::value)
        {
            ...
            std::transform(
                ...
                x_points.begin(),
                y_points.begin(),
                [t_ref](const S& y, const T& x) { return std::log(y) / (x - t_ref); });
        }
        else
        {
    ...
    interpolator_geometric_average() = default;

    QUARISMA_SERIALIZATION_TEMPLATE(interpolator_geometric_average, x_, y_, x_ref_);
};
