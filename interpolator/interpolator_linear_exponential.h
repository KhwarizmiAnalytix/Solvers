...
class interpolator_linear_exponential : public interpolator_linear<Container, T, S>
{
...
public:
     ...
     */
    interpolator_linear_exponential(std::vector<T> x_points, Container y_points)
...
private:
    /**
     * @brief Transform all y values to exp(y) before passing to base constructor
     * @param y_points Original y values
     * @return Transformed y values (exp(y))
     */
    static Container transform_y_values(Container y_points)
    {
        if constexpr (std::is_arithmetic<S>::value)
        {
            std::transform(
                ...
                y_points.end(),
                y_points.begin(),
                [](const auto& y) { return std::exp(y); });
        }
        else
        {
            y_points = exp(y_points);
        }

        return y_points;
    }

    interpolator_linear_exponential() = default;

    QUARISMA_SERIALIZATION_TEMPLATE(interpolator_linear_exponential, x_, y_);
};
