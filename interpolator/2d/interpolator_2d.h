...
public:
    ...
    const Container& z() const noexcept { return z_; }
    Container&       z() noexcept { return const_cast<Container&>(z_); }

    void update(T x, T y, double val)
    {
        const size_t i = find_interval_x(x);
        const size_t j = find_interval_y(y);

        z_.data()[i * size_y() + j] = val;
    }
...
protected:
    ...
    [[nodiscard]] constexpr S get_z(size_t i, size_t j) const noexcept { return z_.at(i, j); }
...
protected:
    ...
    void validate() const
    {
        QUARISMA_CHECK_DEBUG(size_x() == z_.rows(), "x size must match z rows");
        QUARISMA_CHECK_DEBUG(size_y() == z_.columns(), "y size must match z columns");
