...
namespace quarisma
{
class MATH_VISIBILITY plackett_copula : public copula
{
public:
    /**
     * @brief Constructor
     * @param theta Parameter controlling the strength of dependence (theta > 0, theta != 1)
     */
    MATH_API explicit plackett_copula(double theta);

    /**
     * @brief Virtual destructor
     */
    MATH_API ~plackett_copula() override;

    /**
     * @brief Evaluate the Plackett copula function
    ...
    /**
     * @brief Evaluate the Plackett copula density function
     * @param u Vector of uniform variates in [0,1]
    ...
    /**
     * @brief Generate multiple random samples from the Plackett copula using a uniform random number generator function
